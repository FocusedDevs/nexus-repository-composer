...
public class ComposerJsonProcessor {
    ...
    private static final String SEARCH_PATH = "/search.json?q=%query%&type=%type%";
    ...
    private Content buildPackagesJson(final Repository repository, final Set<String> names) throws IOException {
        ...
        packagesJson.put(SEARCH_KEY, repository.getUrl() + SEARCH_PATH);
     ...
     * Merges an incoming set of packages.json files.
     */
    public Content mergeSearchJson(final Repository repository, final List<Payload> payloads) throws IOException {
        Set<String> names = new HashSet<>();
        return buildPackagesJson(repository, names);
    }
